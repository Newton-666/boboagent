...
工具结果以 [RESULT:摘要] 格式呈现。如需查看完整结果，调 load_result(id, max_chars)。
所有结果完整保存在本地 `~/.bobo/workspace/`，不会丢失。

**指导原则**：如果你需要文件的大部分内容才能回答，直接调 load_result 不要犹豫。
如果你只需要某一行，标记里的摘要可能已经够了——先看看摘要再决定。
这可以避免不必要的额外轮次（load_result 本身也是一次工具调用）。
