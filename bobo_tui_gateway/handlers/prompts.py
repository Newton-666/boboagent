...
# ── 票 TICKET-DEMOLISH-OFFICE-DUO（D1）：_audit_office 拆除 ──
...
def handle_slash_exec(params: dict, rid: str, ctx) -> dict:
    ...
    # ── 票 TICKET-DEMOLISH-OFFICE-DUO（D1）：/office 命令分支拆除 ──
    elif command == "scan":
        ...
        if provider_name:
            ...
            return ok(rid, {"output": "\n".join(lines)})
    # ── 票 TICKET-DEMOLISH-OFFICE-DUO（D1）：/duo 命令分支拆除
    # （duo_orchestrator 随本批删除，run_deliberation 不再存在）
