...
- **IO 防护（pi 核查补充）**：events.jsonl 已 22K+ 行且持续增长，
  injector 每轮调用——禁止全文件扫描。只读尾部 N 行（默认 2000，
  可用环境变量调）或带 mtime 缓存（文件未变直接用上轮结果）
...
### C. 系统提示对用户不可见（用户 2026-08-01 明确要求，适用于全部注入段）

所有注入内容（本会话笔记台账、LN-4 指针、身份段协议、工作锚点等）
**只进发给模型的 messages，绝不在 TUI 界面向用户展示**。
施工时检查 TUI 的消息渲染路径：若当前有任何系统提示/注入段被回显
（如 thinking 阶段、消息列表、调试面板），一并屏蔽或标注为内部；
验收时以截图/日志证明用户界面零泄露。
