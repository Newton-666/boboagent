...
### 监控实锤（2026-08-01 从 events.jsonl 提取）

- 累计 60 次压缩中 **18 次 summary_length=0**（30%）——"纯工具记录段"
  兜底路径只归档、零摘要，被压内容对模型永久蒸发。skill 创建会话
  （20260731_225819）12 次压缩里多次命中此路径，与用户失忆报告时间吻合。
- 单会话最高压缩 23 次（20260729_112249）——压缩搅动（churn）：
  keep=30 填满后再压，每个周期都是一次丢失风险。
- 因此本票 A 项锚点必须在**零摘要兜底路径也同样生成**——
  它恰恰是最重要的场景。
