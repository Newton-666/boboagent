...
owner 终裁：**先图后物**——修复路径改为蓝图先行：
1. 把 dist（成品）中已被涂改的功能**反推回 src（蓝图）**——src 必须能代表真实产品；
2. 从 src 重建 dist（vite build），验证重建产物与当前产品一致；
3. 最后才对齐守卫——守卫记的是"蓝图+成品"的新一致状态。
禁止"先改守卫记忆迁就现状"（那是保安改记忆，不是修房子）。
