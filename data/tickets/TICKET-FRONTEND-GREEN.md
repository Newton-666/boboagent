...
## 1b. 治理升级（2026-08-23，宪法 Principle 6 蓝图先行）

owner 终裁（2026-08-23 修正方向）：**先图后物，但蓝图追成品，成品不动**——
当前 dist 是 10 天工作的真实成果，绝对不可回退或改动。
1. **蓝图（src）反推录入成品（dist）的现状功能**——src 记录成品真实状态（选图、computer use 开关等），蓝图追上成品；
2. **守卫记忆更新**到"蓝图=成品"的新一致状态；
3. **成品原样保留，其调整是未来独立任务**（另期执行，不在本票）。
禁止：① 动成品（回退/重建/手改 dist）；② 蓝图未追成品就先改守卫记忆。
