...
## 宏观快照（每批后刷新 · 2026-08-23 当前）

```
阶段 0：D1 拆除 office/duo        ✅（engine 2,545→2,208）
阶段 1：main 修绿（测试债）       🔵 ~85%（61→约 16 测试 + 前端 4 + watchdog 5）
阶段 2：D 类隔离 + 前端票          ⬜
阶段 3：engine 流水线圈            ⬜ ← 目标
```

**到 engine 还差几步**：
1. 查完待查 7 文件（eng1/scan/g3/skill_audit/core_int2/computer_use_core——判断意图错位同类/真 bug/D 类）
2. 全量复跑（约 3 分钟）拿精确剩余数
3. D 类标记隔离（watchdog → `-m live`，本地跳过 CI 跑）
4. 前端票（busy_gate/css×2/v2b3_1，可与 3 并行）

→ 阶段 1 收口 + 阶段 2 完成后，阶段 3（engine 流水线圈）开工。
本次记录批次：B0–B4。下次批次合入时：先写微观条目，再刷新本快照。
