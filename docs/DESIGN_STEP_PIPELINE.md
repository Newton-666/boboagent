...
## 0. 第一原则：模块纯粹性（owner 2026-08-23 定调）

> 拆分依据 = 职责归属，不是代码量。
> engine 有不可缩的身份：状态机迁移 + 上下文组装 + 工具执行环 + 回合收尾控制流。
> `_step()` 里凡属"engine 的政策或观察"（不依赖 engine / 可搬到 engine 外 /
> 对 engine 核心无直接贡献）→ 拆到独立模块；凡属"engine 的组织动作" → 留在 engine。
> "拆出去" ≠ "engine 不管"：执法权留在 engine（唯一调用者 + 放行决定），执法逻辑外移。
> 代码量是"精简"话题（另一个工程动作），不是"拆分"依据——行数与拆分无因果关系。

### 0.1 用第一原则过 8 闸 + 2 段（裁决表，2026-08-23 定稿）

| 组件 | 职责归属 | 裁决 | 理由（运行逻辑） |
|---|---|---|---|
| ⑤ 承诺检测 | 政策（收尾纪律） | **拆** | 纯决策（正则+台账扫描），持有私有计数器 |
| ⑥ 答复质量 | 政策 | **拆** | 纯启发式函数，自持计数器 |
| ⑦ 补账检测 | 政策 | **拆** | `_detect_ledger_backfill(prev,names)` 签名已独立 |
| ⑦' 字段闸 | 政策 | **拆** | L1 pass-with-note，自持计数器 |
| ⑧ 台账未销账 | 政策 | **拆** | 与⑤同构 |
| EXECUTING tracker | 观察（执行后侧写） | **拆（Hook 化）** | 观察者模式，天然可挂载 |
| ③ 沉淀派发 | 组织（回合收尾必须退场） | **留** | fire-and-forget 是 engine 的组织职责 |
| ① 空响应重试 | 组织（控制流，动 current_depth） | **留** | 重试与深度强耦合，是状态机的一部分 |
| ② 验证器 | 组织（check_and_inject 改 history） | **留** | 引擎级动作，动 engine 内脏 |
| RESPONDING 终稿组装 | 组织（写 pending_content） | **留**（展示层债记 backlog） | 本圈不动 |

本圈范围：拆 ⑤⑥⑦⑦'⑧ + tracker（6 个），留 ①②③ + 终稿组装。前置圈（office/duo
拆除）已把原 S2 快照闸带走，无需再议。
