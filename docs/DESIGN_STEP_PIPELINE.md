...
## 0a. 执行策略：先砌墙，后搬房（owner 2026-08-23 定稿）

> 白话模型：engine 是"走廊 + 房间"。走廊 = 核心 loop（想→干→看→收尾的控制流）；
> 房间 = 挂在走廊上的纪律（承诺/质量/台账检查等）。**解耦第一步 = 走廊与房间先分家**。

**顺序（四步）**：
1. **看完全部房间**——把走廊里所有待搬的房列完整清单（数房间，设计墙的依据）；
2. **砌墙**——定"门缝"（房间如何被叫、如何回话：只读快照入、放行/拦下出），立最小骨架
   （走廊调用点 + 房间接口 base class）；契约**对着全部房间一起设计**，接口先保守后放宽；
3. **搬低耦合房间**——旁观记账、纯判断的房间先搬，验证墙质量（连最不粘的都插不顺畅
   = 墙砌歪，趁早改）；
4. **搬高耦合房间**——空响应重试/验证器等与走廊缠最深的最后搬（墙已成熟）。

**反模式**：不砌墙先拆房 = 每间房一个临时接法 → "形式的流水线"（文件分散、耦合原样）。
禁止。

**两把尺子（验收）**：
1. 行为基线 diff=0（6 场景，scripts/step_baseline.py）；
2. **LLM 调用次数不升**（重构后同场景调用数下降或持平——循环交错解开的直接读数；
   重构前先测每回合调用基线）。

---

## 0a.1 房间清单（数房间，2026-08-23，_step 现状 16 个评估对象）

### 走廊入口（控制流，耦合最高——最后动）
| # | 房间 | 职责 | 耦合度 | 依据 |
|---|---|---|---|---|
| C1 | 空响应重试 | 模型返回空 → 重试/报错 | 高 | 动 current_depth（与走廊转动缠在一起） |
| C2 | 验证器 | 声称完成但零工具 → 回注 | 高 | 改 history（引擎级动作） |

### THINKING 收尾段
| # | 房间 | 职责 | 耦合度 | 依据 |
|---|---|---|---|---|
| R1 | 沉淀派发 | 收尾起后台沉淀线程 | 中 | 要起线程（engine 组织动作），决策薄 |
| R2 | 承诺检测 | 未来时承诺 → 回注/熔断 | 低 | 只读快照 + 自持计数器 |
| R3 | 答复质量 | 台账腔/思考落纸 → 回注 | 低 | 只读快照 + 自持计数器 |
| R4 | 补账检测 | 批量建账即全 done → deny | 低 | 只读 prev ledger + 嫌疑 flag |
| R5 | 台账字段闸 | 缺字段 pass-with-note | 低 | 只读 + 自持计数器 |
| R6 | 台账未销账 | 有 pending → 回注/熔断 | 低 | 只读 + 自持计数器 |
| R7 | 无账软放行 | 无账 → 事件记录 | 低 | R6 的尾巴，随 R6 |

### EXECUTING 段
| # | 房间 | 职责 | 耦合度 | 依据 |
|---|---|---|---|---|
| E1 | 编辑冲突检测 | 多编辑同文件 → 回注 | 中 | 改 history 回注（引擎级） |
| E2 | 台账基线快照 | 执行前快照台账 | 中 | 与工具环时序强绑定 |
| E3 | 台账同步+补账嫌疑 | 执行后同步/评估 | 中 | 紧贴工具执行结果 |
| E4 | 全绿销账建议 | 测试全绿 → 建议销账 | 低 | 只读工具结果 + 注入建议 |
| E5 | tracker 记账 | 改动/已读/模式侧写 | 低 | 纯观察，天然可挂载 |

### RESPONDING 段
| # | 房间 | 职责 | 耦合度 | 依据 |
|---|---|---|---|---|
| P1 | 工作区对账 | 收工 git status 注入 | 低 | 纯只读侧写 |
| P2 | 终稿组装 | 台账尾注/交接/思考展示 | 高 | 写 pending_content（展示层债） |

### 附：_step 之外的政策（ENGINE-RESPONSIBILITY B 档，另圈）
- computer use 降级（~90 行，_confirm 侧）→ cu 政策圈
- 循环检测（~60 行）→ 并入 round_tracker
- 沉淀提取（~50 行）→ 搬 skill_sedimenter

### 搬移顺序（低→高，先验证墙）
**第一批（低，6 间）**：R2 承诺 + R3 质量 + R4 补账 + R5 字段 + R6 台账 + E5 tracker
**第二批（中，4 间）**：E4 销账建议 + P1 对账 + R1 沉淀派发 + E1 冲突检测
**第三批（高，4 间）**：E2 基线快照 + E3 台账同步 + C1 空响应重试 + C2 验证器
**另期**：P2 终稿组装（展示层债，backlog）
