...
> v3.6 · 2026-08-14 · 随每票终审合并滚动更新；发现新问题/新想法 → 补进本文件对应梯队
...
| 5 | DESK-V2D 美学微模块票（owner 12:19 定调：**大 CSS 骨架一概不动，只做小模块**；Hermes 建议过滤后按 impact 从小到大） | ①字体：回复正文 Charter 衬线（只改 font-family 一处变量）②橙色印章语义（橙=bobo 手笔/灰=机器状态，纯 token 语义归位）③排版细节：中西文混排间距+中标点悬挂+引用块 serif（中楷体/西 Charter）④药丸升认知状态条（水位+记忆数+工具数）；不做：骨架/配色体系/浅色纸感 | 📋 V2C1+C2 之后 |
