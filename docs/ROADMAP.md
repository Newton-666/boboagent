...
| 票 | 干什么 | 状态 |
|---|---|---|
| **P0-1 记忆六类分类 + Memory 模块 UI（2026-08-19 开票，自进化系统第一票）** | 自进化施工开始（快照 snapshot/pre-self-evolving-20260818 已打）。记忆六类（USER_PREF/RULES/FACT/ACHIEVEMENT/LESSON/GOAL）+ 656 条历史迁移（确定性启发式+人工抽查）+ 指针校验 + 侧栏 Memory 模块（diff 增删/token 统计/手动编辑）。独立价值：解决现有 656 条失控 + 空分类 | 📋 排队（票 docs/tickets/TICKET-P0-1.md） |
