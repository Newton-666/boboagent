...
| **GUI-F29 聊天区虚拟滚动 + 滚动丝滑（2026-08-19 开票，长会话 137 崩溃治本）** | owner 反馈长会话（~200 条）renderer 内存 3.5GB → SIGKILL 窗口消失循环。根因：chat DOM 无限累积无上限。修：虚拟滚动（可视区±缓冲只渲染需显示的 + transform GPU 合成）+ 消息高度缓存 + DOM 上限 500 + pendingMessages 上限。Owner 定调：丝滑不卡即可（≥50fps），不追 120Hz，不出错 > 极致性能，全部既有行为零回归 | 📋 排队（票 docs/tickets/TICKET-GUI-F29.md） | | | | | | |
