...
> v4.6 · 2026-08-14 · 随每票终审合并滚动更新；发现新问题/新想法 → 补进本文件对应梯队
...
| 票 | 干什么 |
|---|---|
| GOV-1 能力内化（owner 15:15 拍板：训练成果必须开箱自带，不能只在 Kimi 话术里） | ①教训册/工作流纪律进系统提示词自动注入（引擎级触发，不靠叮嘱）②收工自审 diff 固化成提示词级标准动作（实证：bobo 自审 F12 自抓 4 bug）③EV-2 加"新人测试"：零上下文 bobo 开箱验证查码/改码/自验/全量回归 |
