...
| **GUI-F30 桌面端关闭后 gateway 不刷新（2026-08-19 开票，owner 反馈）** | Cmd+Q 关闭窗口→重开，gateway 仍旧进程不加载新代码（复用不 spawn + stopBackend 只杀自 spawn，注释 245-246 明说杀不到外部）。owner 诉求：关掉再打开=后端刷新。方案 C（推荐）：Cmd+Q 必杀 gateway+每次启动重 spawn；方案 B（备选）版本感知复用。会话历史不丢（磁盘持久化），进行中任务中断可预期 | 📋 排队（票 docs/tickets/TICKET-GUI-F30.md） |
| **GUI-F29 聊天区虚拟滚动 + 滚动丝滑（2026-08-19 开票，长会话 137 崩溃治本）** | owner 反馈长会话（~200 条）renderer 内存 3.5GB → SIGKILL 窗口消失循环。根因：chat DOM 无限累积无上限。修：虚拟滚动（可视区±缓冲只渲染需显示的 + transform GPU 合成）+ 消息高度缓存 + DOM 上限 500 + pendingMessages 上限。Owner 定调：丝滑不卡即可（≥50fps），不追 120Hz，不出错 > 极致性能，全部既有行为零回归 | 📋 终审返工（票 docs/tickets/TICKET-GUI-F29.md）
| **RWORK-F29 虚拟滚动终审返工（2026-08-19 开票，F29 首轮未通过）** | 16 测试失败（f6d 适配 + TEL-8 登记）+ 3 行为回归：①工具卡错乱（多工具并行 + 窗口化回收后 updateToolResult 找不到卡 → diff 挂错 + task_ledger 无友好名）；②思考折叠失效（liveRecycle 不清 thinkBoxEl）；③diff 不常驻（被回收/折叠）。Owner：不出错 > 极致性能，全部零回归 | 📋 返工（票 docs/tickets/TICKET-RWORK-F29.md）
| **COST-7 HTTP 400 reasoning_content 系统性根治（2026-08-19 开票，owner 定调最后信任票）** | 多因素组合根治（非补丁）：F2 引擎 5 处 user 注入改 system（消双 user 夹工具轮）+ F3 压缩路径保留 thinking + F4 孤儿清洗占位补 reasoning_content + F5 thinking 采集完整性 + 核心实验（空串 vs 原值策略定案，真实 API 验证）。前票只修 F2 一分支（动态块），剩 5 处 + 3 因素未碰。验收：全量 0 失败 + 缓存不塌 + 真实长会话不 400。若仍连锁出问题 → 回溯自进化基底 snapshot/pre-self-evolving-20260818 | 📋 排队（票 docs/tickets/TICKET-COST-7.md） | | | | | | | | |
