...
| 2 | DESK-V4B 小组件会话钉选（owner 23:47 拍板：**用途升级=第二只眼睛**，提到 CLI 之前立刻干） | 主窗聊 session B 时小组件钉住 session A 的实况，实现"同时干两个会话"；事件流按 sid 过滤（F10 地基现成）；小组件顶部极小会话指示；主窗会话行内"投影到小组件"入口 + 小组件上循环切换运行中会话；不钉则默认跟当前会话（V4 现行为）；零干涉铁律不破（只读，插手=点击跳主窗）；**追加⓪（owner 23:51 实弹抓 bug）：忙碌态按会话隔离——A 运行中 B 的输入区/发送键/状态栏必须是空闲可用态（现状=全局忙碌锁死 B，直接卡死双会话场景）；红点/飞态只反映当前显示会话** | 📋 下一票（先于 CLI） |
