...
| 序 | 票 | 干什么 | 状态 |
|---|---|---|---|
| 1 | LEDGER-1 | 台账机制重构：收工引擎自动对账（git 实况注入）+ deny 降本（精简补正不再全上下文重跑）+ 建账提醒每回合至多一次 + 收工汇报质量标准（完成项逐条带证据，禁一句话交底） | 🔧 已派单，bobo 施工中 |
| 2 | F6D | 显示层补丁：①相邻判定跳过状态行（F6B/F6C 漏网）②写类工具（编辑/diff）的思考不吞并、全程摊开（创作痕迹摊开看，翻资料过程收进柜） | 📋 票已写 |
| 3 | F8 | 历史会话恢复 diff 高亮 + 思考过程（仅桌面端；思考需新增落盘，历史会话救不回） | 📋 票已写 |
