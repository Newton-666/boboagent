...
def test_prompt_budget_event(library, no_skills, tmp_path, monkeypatch):
    ...
    sec = ev["sections"]
    # 七段齐全（票 TICKET-E3b：+guidance 预付层；票 O4-2：+office 会话告示；票 G1-1：+selfmap L0 常驻）
    assert set(sec.keys()) == {"identity", "memory", "skills", "note_pointers", "guidance", "office", "selfmap"}
