...
def test_tel_8_zero_interference():
    ...
    for ln in changed:
        ...
        if ln == "core/intent.py":
            r_int = subprocess.run(["git", "diff", "main", "--", ln], capture_output=True, text=True, cwd=ROOT)
            assert "COST-3" in r_int.stdout, f"{ln} 缺 COST-3 特批标记，未授权改动被拦截"
            continue
        # 票 TICKET-COMPUTER-USE-ROUTE 特批：core/proactive.py + core/skill_loader.py
