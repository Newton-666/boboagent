...
on('tool.start', function(data) {
  ...
  // TICKET-DESK-WORKER-VISIBLE（thinking 空框修复，同文件登记标记）：收束内容
  // 取框内实际内容——思考走 reasoningText、正文走 thinkText，模型只思考未发
  // 正文时 thinkText 为空，直接用 thinkText 收束会把显示中的思考覆盖成空白
  // 折叠段（千问/DeepSeek 工具轮复现）；两者皆空则移除空框，不残留空白 thinking。
  if (thinkBoxEl) {
    var shownText = thinkText || reasoningText;
    if (shownText) {
      collapseThinkBox(thinkBoxEl, shownText);
    } else {
      thinkBoxEl.remove();
    }
    thinkBoxEl = null;
